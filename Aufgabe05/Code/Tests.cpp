...
bool testAdd(){
    ...
    int atcoCommandsCnt = 0;
    int atcoCommandsMax = 0;

    AtcoCommand newCmd{}, newCmd2{};
    newCmd.filename = "testFilename"; newCmd.wordSequence = "testWordSequence";
    newCmd2.filename = "testFilename2"; newCmd2.wordSequence = "testWordSequence2";
    Add(atcoCommands, atcoCommandsCnt, atcoCommandsMax, newCmd);
    Add(atcoCommands, atcoCommandsCnt, atcoCommandsMax, newCmd2);

    if(atcoCommandsCnt != 2)
        return false;
    if(atcoCommands[0].filename != "testFilename" || atcoCommands[0].wordSequence != "testWordSequence")
        return false;
    if(atcoCommands[1].filename != "testFilename2" || atcoCommands[1].wordSequence != "testWordSequence2"){
        return false;
    }
    return true;
}
bool testResize(){
    int atcoCommandsMax = 5;
    int atcoCommandsCnt = 0;
    auto * atcoCommands = new AtcoCommand[atcoCommandsMax];
    //calls resize 3 times, the last time only one element is actually filled and the rest is empty
    for(int i = 0; i < 16; i ++){
        AtcoCommand newCmd;
        newCmd.filename = "file "+ std::to_string(i);
        newCmd.wordSequence = "test word sequence";
        Add(atcoCommands, atcoCommandsCnt, atcoCommandsMax, newCmd);
    }

    if(atcoCommandsMax != 20 || atcoCommandsCnt != 16)
        return false;
    for(int i = 0; i < 16; i ++){
        if(atcoCommands[i].filename != "file "+ std::to_string(i) || atcoCommands[i].wordSequence != "test word sequence"){
            return false;
        }
    }
    for(int i = 16; i < 20; i++){
...
bool testFillAtcoCommands() {
    ...
    FillAtcoCommands(atcoCommands, atcoCommandsCnt, atcoCommandsMax, inFile);
    for (int i = 0; i < 6; ++i) {
...
bool testCountWordOccurences() {
    ...
    std::ifstream inFile("WordSeqPlusCmds.txt");
    FillAtcoCommands(atcoCommands, atcoCommandsCnt, atcoCommandsMax, inFile);
    CountWordOccurences(atcoCommands, atcoCommandsCnt, wordSeqCntMap);
...
bool testCountCmdOccurences() {
    ...
    CountCmdOccurences(atcoCommands, atcoCommandsCnt, cmdSeqCntMap, allowedCmdSet);

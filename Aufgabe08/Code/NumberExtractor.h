#ifndef AUFGABE8_NUMBEREXTRACTOR_H
#define AUFGABE8_NUMBEREXTRACTOR_H
#include <map>
#include <string>
#include <vector>
...
public:
  // this inner class only responsibility is saving the extracted numbers and
  // the string and exposing it as int, double. Also exposes functions to check
  // if the extracted number is valid and to get the string which was processed.
  // Note: The processing of the word sequence happens in the parent class
  // NumberExtractor
  class ExtractedNumber {

  public:
    /**
     * @param astr_seqOfNumbers Only sequence of the string that contains the
     * words that could be processed to numbers
     * @param numberToStore
     * @param numAfterDeci Which number comes after the decimal point
     */
    ExtractedNumber(std::string astr_seqOfNumbers = "", int numberToStore = -1,
                    int numAfterDeci = -1);

    int GetExtractedNumberAsInt() const { return mi_number; }
    double GetExtractedNumberAsDouble() const;
    bool IsExtractedNumberValid() const { return mi_number >= 0; }
    std::string GetNumberStringSeq() const { return mstr_numberSeq; }
  ...
  private:
    ...
    int mi_number = -1;
    //! the extracted number after word "decimal" etc. if we have extracted a
    //! double
...
public:
  ...
  // starting after the word startIndexExtractionAfter, first word starts at
  // index 0
  ...
  //! Extracts the next complete number from sequence of words provide in
  //! constructor
...
private:
  std::map<std::string, std::string>
      dictionaryMap; // read in from file or define in place ?
  std::vector<std::string>
      fullWordSeq; // could also use DynArray<std::string> here
  std::vector<ExtractedNumber> m_extractedNumberSeq;
  int startIndexExtractionAfter; // start after this index! -> always one lower
                                 // than the index that you want to start from

...
bool testTrim() {
  ...
  std::string d = " b";
  std::string e = " ";
  if (Trim(a) != "aabb ccdd" || Trim(b) != "aaf" || Trim(c) != "a" ||
      Trim(d) != "b" || !Trim(e).empty()) {
    return false;
  }
  return true;
}
bool testFileNameSplit1() {
  ...
  if (fileName1.GetDay() != 12 || fileName1.GetMonth() != 7 ||
      fileName1.GetYear() != 2016 || fileName1.GetHour() != 8 ||
      fileName1.GetMinute() != 33 || fileName1.GetSeconds() != 49 ||
      fileName1.GetMilliseconds() != 1)
    return false;
  return true;
}
...
bool testDynArrayAdd() {
  ...
  if (dynArr.Get(0) != "test" ||
      dynArr1.Get(0).GetWordSequence() != "test word sequence")
    return false;
  return true;
}
...
bool testDynArrayResize() {
  ...
  if (dynArr.GetLimit() != 7 || dynArr.GetCntElements() != 3 ||
      dynArr.Get(2) != "string 2")
    return false;
  return true;
}
bool testDynArrayCopyConstructor() {
  ...
  if (dynArr.GetArrPtr() == dynArr2.GetArrPtr() ||
      dynArr.GetCntElements() != dynArr2.GetCntElements())
    return false;
  return true;
}
bool testDynArrayAssignmentOperator() {
  ...
  if (dynArr.GetArrPtr() ==
      dynArr2.GetArrPtr()) // check if the pointers to the array of AtcoCommand
                           // are different
    return false;
  for (int i = 0; i < dynArr2.GetCntElements(); i++) {
    // check for each AtcoCommand (which holds an DynArray<std::string>) if the
    // pointer to its DynArray<std::string> are different (deep copied)
...
bool testAtcoCmdsReadFromFile() {
  ...
  if (atcoCmds.GetCountedUtterances() != 11 ||
      atcoCmds.GetUtterancesLimit() != 15)
    return false;
  atcoCmds.ReadFromFile(inFile2);
  atcoCmds.ReadFromFile(inFile3);
  if (atcoCmds.GetCountedUtterances() != 15 ||
      atcoCmds.GetUtterancesLimit() != 15)
    return false;
  for (int i = 0; i < atcoCmds.GetCountedUtterances(); i++) {
    AtcoCommand atcoCommand = atcoCmds.Get(i);
    if (atcoCommand.GetFileName().GetName().empty() ||
        atcoCommand.GetWordSequence().empty() ||
        atcoCommand.GetCommandsPtr() == nullptr)
      return false;
  }
...
bool testAtcoCmdsMoreThanSixCommands() {
  ...
  for (int i = 1; i < 9; i++) {
    ...
    std::string cmpCommand =
        Trim("BAW52CV CLIMB 12" + std::to_string(i) + " FL  ");
...
bool testFillAllowedCommands() {
  ...
  if (!inTypes) {
    std::cerr << "Die angegebene Datei für erlaubten Commands konnte nicht "
                 "gefunden werden.";
    return false;
  }
  ...
  const bool is_in12 =
      allowedCmdSet.find("CALL_YOU_BACK") != allowedCmdSet.end();
...
bool testFillAllowedCommands1() {
  ...
  const bool is_in4 =
      allowedCmdSet.find("INFORMATION QNH") != allowedCmdSet.end();
  const bool is_in5 =
      allowedCmdSet.find("INFORMATION ATIS") != allowedCmdSet.end();
  ...
  if (!is_in1 || !is_in2 || !is_in3 || !is_in4 || !is_in5 || !is_in6 ||
      !is_in7 || !is_in8)
    return false;
  return true;
}
...
bool testCountCmdOccurences() {
  ...
  std::ifstream inFile(TestFilesPath + "WordSeqPlusCmds.txt"),
      allowedCmds(TestFilesPath + "expectedTypes.txt");
  atcoCmds.ReadFromFile(inFile);
...
bool testCountCmdOccurences1() {
  ...
  AllowedCmdSet allowedCmdSet;
  CmdCntMap cmdSeqCntMap;
  std::ifstream inFile(TestFilesPath + "WordSeqPlusCmds2.txt"),
      allowedCmds(TestFilesPath + "expectedTypes2.txt");
  atcoCmds.ReadFromFile(inFile);
  FillAllowedCommands(allowedCmdSet, allowedCmds);
  CountCmdOccurences(atcoCmds, cmdSeqCntMap, allowedCmdSet);
...
bool testOrderWordOccurences() {
  ...
  std::advance(iter, 1);
  fp10 = *iter;
  if ((fp1.first != "one" && fp1.second != 8) ||
      (fp2.first != "radar" && fp2.second != 7) ||
      (fp3.first != "six" && fp3.second != 5) ||
      (fp4.first != "two" && fp4.second != 5) ||
      (fp5.first != "zero" && fp5.second != 5) ||
      (fp6.first != "alfa" && fp6.second != 4) ||
      (fp7.first != " contact" && fp7.second != 4) ||
      (fp8.first != "flight" && fp8.second != 4) ||
      (fp9.first != "level" && fp9.second != 4) ||
      (fp10.first != "lusthansa" && fp10.second != 4))
    return false;
  return true;
}
...
bool testOrderWordOccurences1() {
  ...
  std::advance(iter, 1);
  p10 = *iter;
  if ((p1.first != "two" && p1.second != 3) ||
      (p2.first != "charlie" && p2.second != 2) ||
      (p3.first != "climb" && p3.second != 1) ||
      (p4.first != "five" && p4.second != 1) ||
      (p5.first != "flight" && p5.second != 1) ||
      (p6.first != "good" && p6.second != 1) ||
      (p7.first != "level" && p7.second != 7) ||
      (p8.first != "morning" && p8.second != 1) ||
      (p9.first != "speedbird" && p9.second != 1) ||
      (p10.first != "zero" && p10.second != 1))
    return false;
  return true;
}
...
bool testOrderCmdOccurences() {
  ...
  std::advance(iter, 1);
  p4 = *iter;
  if ((p1.first != "CLEARED" && p1.second != 2) ||
      (p2.first != "DIRECT_TO" && p2.second != 2) ||
      (p3.first != "CLEARED ILS" && p3.second != 1) ||
      (p4.first != "DESCEND" && p3.second != 1))
    return false;
  return true;
}
...
bool testOrderCmdOccurences1() {
  ...
  std::advance(iter, 1);
  p4 = *iter;
  if ((p1.first != "CLIMB" && p1.second != 2) ||
      (p2.first != "CLEARED ILS" && p2.second != 1) ||
      (p3.first != "INFORMATION ATIS" && p3.second != 1) ||
      (p4.first != "INFORMATION QNH" && p4.second != 1))
    return false;
  return true;
}

NumberExtractor testForIntNumbers(std::string &wordSeq,
                                  std::vector<int> &expNumbers, bool &success) {
  NumberExtractor numEx(wordSeq);
  numEx.PerformFullExtraction();
  success = true;
  int expectedLength = static_cast<int>(expNumbers.size());
  if (expectedLength > numEx.GetExtractedNumbersCnt()) {
    success = false;
    return numEx;
  }
...
NumberExtractor testForDecimalNumbers(std::string &wordSeq,
                                      std::vector<double> &expNumbers,
                                      bool &success) {
  NumberExtractor numEx(wordSeq);
  numEx.PerformFullExtraction();
  success = true;
  ...
  for (int i = 0; i < expectedLength; ++i) {
    if (!numEx.IsNumberDouble(i) ||
        numEx.GetNumberAsDouble(i) != expNumbers.at(i)) {
      success = false;
      break;
    }
  }
...
bool testExtractDecimalNumbers() {
  std::string wordSeq =
      "contact director one one nine decimal eight four goodbye";
  ...
  std::string wordSeq3 = "contact director one one nine point two zero";
  std::vector<double> expNumbers3 = {119.20};
  bool b_success3 = true;
...
bool testAtcoCommandDetermineCallsign() {
  AtcoCommand ac1, ac2, ac3, ac4, ac5;

  ac1.DetermineCallSign(
      "oscar echo india november kilo direct whisky whisky nine eight five");
  ac2.DetermineCallSign(
      "good morning lufthansa one two bravo descent eight zero");
  ac3.DetermineCallSign(
      "gruess gott ryan_air seven seven delta kilo in radar contact");
  ac4.DetermineCallSign("gruess gott standby");
  ac5.DetermineCallSign("gruess gott austrian triple seven sierra identified "
                        "climb flight level two three zero");
  if (ac1.GetCallSign() != "OEINK" || ac2.GetCallSign() != "DLH12B" ||
      ac3.GetCallSign() != "RYR77DK" || ac4.GetCallSign() != "NO_CALLSIGN" ||
      ac5.GetCallSign() != "AUA777S")
    return false;
  return true;
}
bool testReadUtteranceCheckCallsign() {
  std::vector<std::string> expectedValues = {
      "BAW52CV", "SAS1767",     "ABP61",  "BAW52CV", "BAW52CV",
      "ABP60",   "NO_CALLSIGN", "DLHABD", "DLHABD",  "DLHABD"};
  std::string filePath = TestFilesPath + "WordSeqPlusCmds.txt";
  if (!ReadUtteranceCheckCallsign(filePath, false, expectedValues))
    return false;
  return true;
}
bool testReadUtteranceCheckCallsign2() {
  std::vector<std::string> expectedValues = {
      "NO_CALLSIGN", "SAS1767", "ABP62",  "BAW52CV", "BAW52CV",
      "ABP60",       "TEST",    "DLHABD", "DLHABD",  "DLHABD"};
  std::string filePath = TestFilesPath + "WordSeqPlusCmds.txt";
  if (ReadUtteranceCheckCallsign(filePath, false, expectedValues))
    return false;
  return true;

#ifndef AUFGABE8_ATCOCOMMAND_H
#define AUFGABE8_ATCOCOMMAND_H
...
public:
   ...
   * assignment of this->commands = copy.commands calls the assignment operator
   * of the DynArray<std::string>, which then correctly deletes the old memory
   * and allocates the new array of strings.
   */
  /**
   * The default constructor is only needed so that in Resize() in DynArray.cpp
   * we can create an array of AtcoCommands on the heap, which will get filled
   * afterwards with the contents of the old array. This is why this class also
   * has no setters, because the the default constructor is only used when
   * AtcoCmds allocate an Array (Heap Memory) of AtcoCommands (in its
  ...
  void DetermineCallSign(std::string const &wordSequence);
...
private:
  ...
  std::string callSign = "NO_CALLSIGN";
  DynArray<std::string> commands;
};

#endif // AUFGABE8_ATCOCOMMAND_H

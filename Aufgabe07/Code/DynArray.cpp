...
template <class T>
DynArray<T>::DynArray(const int &paramLimit)
{
    limit = paramLimit;
}
template <class T>
DynArray<T>::DynArray(const DynArray<T> &copy)
{
...
void DynArray<T>::Add(const T &element)
{
    ...
    arrPtr[cntElements] = element; //no need to increment before, because arrays start at 0. Assignment operator of T called
    cntElements += 1;
}
template <class T>
void DynArray<T>::Set(const T &element, const int &pos)
{
    CheckAgainstLimit(pos);
    arrPtr[pos] = element; //assignment operator of T
}
template <class T>
//normally this should also return const, but because of the possibility that an exception gets thrown it does not
...
template <class T>
void DynArray<T>::CheckAgainstLimit(const int &pos)
{
    if (pos >= limit)
        throw std::out_of_range("invalid index");
}
template <class T>
const T *DynArray<T>::GetArrPtr() const
{
    return arrPtr;
}

template <class T>
int DynArray<T>::GetCntElements() const
{
    return cntElements;
}
template <class T>
int DynArray<T>::GetLimit() const
{
    return limit;
}

...
darzustellen, wenn man Ausgaben mit cout durchf�hrt.
Beispiel:
...
inline short SetAndRememberColor(short new_color)
{
   return new_color;
}

template <typename T>
inline void printScreenColorOnceVal(std::ostream &, short, T)
{
}

template <typename T>
inline void printScreenColorOnceRef(std::ostream &, short, const T &)
{
}
...
inline WORD SetAndRememberColor(WORD new_color)
{
   WORD old_color = 0;
   HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
...
template <typename T>
inline void printScreenColorOnceVal(std::ostream &s, WORD col, T text)
{
...
template <typename T>
inline void printScreenColorOnceRef(std::ostream &s, WORD col, const T &text)
{
   WORD oldColor = SetAndRememberColor(col);
   s << text;
   s << color(oldColor);
